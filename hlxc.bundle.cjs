...
var Consola = class _Consola {
  ...
  constructor(options = {}) {
    ...
    for (const type2 in types) {
      ...
      this[type2] = this._wrapLogFn(defaults);
      this[type2].raw = this._wrapLogFn(
        defaults,
        true
      );
    }
   ...
   */
  wrapConsole() {
    for (const type2 in this.options.types) {
      if (!console["__" + type2]) {
        console["__" + type2] = console[type2];
      }
      console[type2] = this[type2].raw;
    }
  }
  /**
  ...
  restoreConsole() {
    for (const type2 in this.options.types) {
      if (console["__" + type2]) {
        console[type2] = console["__" + type2];
        delete console["__" + type2];
      }
    }
  }
  /**
  ...
  _wrapStream(stream, type2) {
    ...
    stream.write = (data) => {
      this[type2].raw(String(data).trim());
    };
  }
  /**
  ...
  mockTypes(mockFn) {
    ...
    for (const type2 in this.options.types) {
      this[type2] = _mockFn(type2, this.options.types[type2]) || this[type2];
      this[type2].raw = this[type2];
    }
  }
...
${indent}`);
  ...
  formatLogObj(logObj, opts) {
    ...
    const type2 = this.formatType(logObj, isBadge, opts);
    const tag = logObj.tag ? colors.gray(logObj.tag) : "";
    let line;
    const left = this.filterAndJoin([type2, characterFormat(message)]);
...
var isInteger = (c3) => {
  let x2 = parseInt(c3);
  return !isNaN(x2) && Number.isInteger(x2);
};
var Token = class {
  constructor(type2 = null, value = null) {
    this.type = type2;
    this.value = value;
  }
};
var Lexer = class {
  ...
  constructor(source) {
    ...
    this.pos = 0;
    for (let t2 of TOKENS) this.tokens[t2] = (v2) => {
      return new Token(t2, v2);
    };
  }
  /**
  ...
  */
  ...
  getNextToken() {
    ...
    if (this.pos > this.src.length - 1) {
      ...
      token = currentChar == ":" ? this.tokens.REG(result) : this.tokens.MEM(result);
    } else {
      let keyword = "";
      while (currentChar != " ") {
        keyword += currentChar;
...
// src/lib/parser.mjs
var padBinaryString = (bits, bStr) => {
  let padded = "";
  for (let i2 = 0; i2 < bits; i2++) padded += "0";
  padded += bStr;
  115;
...
var Instruction = class {
  ...
  setRaw(hi, lo) {
    ...
    this.aluCtrl = lo >> 5 & 7;
    this.regAddr = lo >> 2 & 7;
    this.writeFlag = lo >> 1 & 1;
    this.clearFlag = lo & 1;
  }
  build() {
    let resultHigh = (!this.immFlag ? this.memAddr : this.immValue) << 1 | this.immFlag;
    let resultLow = 0;
...
var Parser = class {
  ...
  lookAhead(n2 = 1) {
    ...
    if (this.pos + n2 < this.tokens.length) {
      for (let i2 = 0; i2 < n2; i2++) {
        let nextToken = this.tokens[start + i2];
  ...
  parseKeyword(token) {
    ...
    if (ALU_MAP[token.value] !== void 0) {
      line += `${addressType}(${address})`;
      inst.aluCtrl = ALU_MAP[token.value];
      this.advance();
    } else if (SPECIAL_INST_MAP[token.value] !== void 0) {
...
var CodeGen = class {
  ...
  async build(outFile) {
    ...
    await (0, import_node_fs.writeFile)(outFile, outJson, () => {
    });
  }
};
...
var genStage = async (instructions, outFile) => {
  let codeGen;
  try {
